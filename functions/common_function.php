...
// getting product
 function getproducts(){
    ...
    // checking the condition to isset or not 
    if(!isset($_GET['category'])){
      if(!isset($_GET['brand'])){


    $select_query = "Select * from `products` order by rand() LIMIT 0,20"; //limit 0,12, HOW may product in display in home
...
while ($row = mysqli_fetch_assoc($result_query)) {
    ...
    echo "<div class='col-md-4 my-2 '>
          ...
          <div class='card-body'>
            ...
            <p class='card-text text-danger fs-5  mt-0 '>Price: <span class='text-decoration-line-through text-secondary me-1 '> $product_price tk</span> <span class='fw-bold'>";
...
<div class='card-body'>
 ...
 // geting the unique categories function
...
// geting the unique Brands function
...
$num_of_rows=mysqli_num_rows($result_query);
if($num_of_rows==0){
echo "<h2 class='text-center text-danger'>This brand not abailable for service</h2>";
}
...
function userData($id){
  global $con;
  $query = "SELECT fname,lname,username,email,phone_no,address  FROM `users` WHERE (`username`='$id' OR `email`='$id' OR `id`= '$id')"; 
